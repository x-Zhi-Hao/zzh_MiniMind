...
def precompute_freqs(
    ...
    if rope_scaling is not None:
        ...
        # orig_max: 模型预训练时的原始最大长度（例如 Llama-2 �? 2048 �? 4096�?
        # factor: 要扩展的倍数 s (比如�? 2k 扩展�? 32k，factor 就是 16)
        # beta_fast (对应论文中的 α): 高频边界，波长比例大于此值的维度不缩�?
        ...
        if end / orig_max > 1.0:
            # 3. 使用前文推导的公式，定义波长比例 b 到维度索�? i 的映射函�?
            ...
            # 5. 计算混合因子 γ (Ramp)
            # �? low 之前，ramp �? 0；在 high 之后，ramp �? 1；在 low �? high 之间，线性过渡�?
            ...
            # �? ramp=0 时（高频）：系数�? 1，保持原频率不变�?
            # �? ramp=1 时（低频）：系数�? 1/factor，即对频率进行线性插值缩放�?
            # ramp�?0-1之间时：平滑过渡�?
            freqs = freqs * (1 - ramp + ramp / factor)

    # 7. 根据目标长度 end，生成位置索引向�? t
    t = torch.arange(end, device=freqs.device)

    # 8. 计算外积：将位置 t 与处理好的频�? freqs 相乘，得到每个位置的旋转角度 θ
    freqs = torch.outer(t, freqs).float()

    # 9. 计算 Cos �? Sin，并应用注意力补偿系�? (attn_factor)
...
class MoEFeedForward(nn.Module):  # ！修正：原MoEFeedForaward拼写错误
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # 每个token安排num_experts_per_tok个专家处�?
    ...
    def moe_infer(self, x, flat_expert_indices, flat_expert_weights):
        ...
        # 对专家索引进行排序，最后是[0,0,0,1,1,2,2,2,...]这样的顺�?
        # 分拣
